#!/usr/bin/env python3
"""
Run sample-headers experiments (headers only on sampled allocations)
"""
...
def run_experiment(workload, scheme, run_num, base_dir, workload_script):
    ...
    env["SAMPLER_LIB"] = str(base_dir / "libsampler_sample_headers.so")
    
    if "HASH" in scheme:
        env["SAMPLER_HASH_MASK"] = DEFAULT_HASH_MASK
    if "POISSON" in scheme or "EBPF" in scheme:
...
def main():
    parser = argparse.ArgumentParser(description="Run sample-headers experiments")
    ...
    lib_path = script_dir / "libsampler_sample_headers.so"
    ...
    print("="*60)
    print("Sample-Headers Experiments")
    print("(Headers only on sampled allocations)")
    print("="*60)
    ...
    print("\nNext steps:")
    print("  1. python3 aggregate_sample_headers_results.py")
